...
for op_count in op_counts:
    ...
    for thread in threads:
        ...
        reads_normal= reads_normal[reads_normal['latency(us)'] < 10000].count()
        reads_slow = reads[reads['latency(us)'] > 10000].count()

        scans_fast = scans[scans['latency(us)'] < 1000]
        scans_normal = scans[scans['latency(us)'] > 1000]
        scans_normal = scans_normal[scans_normal['latency(us)'] < 10000].count()
        scans_slow = scans[scans['latency(us)'] > 10000].count()

        updates_fast = updates[updates['latency(us)'] < 1000]
        updates_normal = updates[updates['latency(us)'] > 1000]
        updates_normal = updates_normal[updates_normal['latency(us)'] < 10000].count()
        updates_slow = updates[updates['latency(us)'] > 10000].count()
